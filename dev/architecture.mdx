...
description: Deep dive into Pipelines as Code architecture, components, and design patterns
---

Pipelines as Code (PAC) is built on top of Tekton Pipelines and provides a GitOps-native CI/CD experience. This guide explains the architecture, components, and how they work together.
...
```mermaid
graph TB
    A[Git Provider<br/>GitHub/GitLab/Forgejo/Bitbucket] -->|Webhook Event| B[Webhook Handler]
    B --> C[Controller]
    C -->|Create| D[PipelineRun]
    D -->|Monitor| E[Watcher]
    E -->|Report Status| A
    C -->|Read| F[Repository CRD]
    C -->|Validate| G[ACL & Permissions]
    C -->|Resolve| H[Tasks from Hub]
    
    subgraph "Pipelines as Code"
        B
        C
        E
    end
...
Here's how a typical pull request event flows through the system:
...
- Repository CRD status is updated
...
The `Repository` Custom Resource Definition (CRD) is the central configuration object:
...
See [Concurrency Flow](/dev/flows-diagram#concurrency-flow) for details.
...
<Tabs>
  ...
  **Authentication**:
  ...
  <Tab title="Forgejo">
  **Package**: `pkg/provider/gitea` (shared with Gitea)
  ...
  **Authentication**:
  ...
  <Tab title="Bitbucket">
  **Package**: `pkg/provider/bitbucketcloud`, `pkg/provider/bitbucketserver`
